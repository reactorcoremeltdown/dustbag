...
packages: repos
	dpkg-query -s $(PACKAGES) > /dev/null || DEBIAN_FRONTEND=noninteractive apt-get install -y $(PACKAGES)
	pip3 install --upgrade youtube-dl
	@echo "$(ccgreen)Installing Debian packages completed$(ccend)"

packages_lite: repos
	dpkg-query -s $(PACKAGES_LITE) > /dev/null || DEBIAN_FRONTEND=noninteractive apt-get install -y $(PACKAGES_LITE)
	@echo "$(ccgreen)Installing Debian packages completed$(ccend)"
