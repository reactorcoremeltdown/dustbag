...
EXPIRY_DATE=`whois "${OPTION}" | grep 'Registry Expiry Date' | cut -f 2- -d ':'`
...
if test "${DELTA}" -lt 6; then
    warning "The domain ${OPTION} expires in ${DELTA} days.\\nGo to https://namecheap.com to verify renewal status."
elif test "${DELTA}" -lt 3; then
    fail "The domain ${OPTION} expires in ${DELTA} days!\\nGo to https://namecheap.com ASAP to fix renewal process!"
else
    ok "The domain ${OPTION} expires in ${DELTA} days."
fi
