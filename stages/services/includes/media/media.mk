...
davfs2:
	...
	bash $(ROOT_DIR)/templates/davfs2/secrets.sh
	install -D -m 644 $(ROOT_DIR)/files/etc/systemd/system/davfs2-mounts/* /etc/systemd/system
...
podsync:
	...
	install -D -m 755 -v $(ROOT_DIR)/files/usr/local/bin/podsync /usr/local/bin
	...
	install -D -m 644 -v $(ROOT_DIR)/files/etc/systemd/system/podsync.service /etc/systemd/system
	systemctl daemon-reload
	systemctl enable podsync.service
	bash $(ROOT_DIR)/templates/podsync/podsync.toml.sh $(ROOT_DIR)/variables/services.json
	systemctl restart podsync.service
	@echo "$(ccgreen)Setting up podsync completed$(ccend)"

dave: davfs2
	install -D -m 755 $(ROOT_DIR)/files/usr/local/bin/dave /usr/local/bin
	install -D -m 644 -v $(ROOT_DIR)/files/etc/systemd/system/dave.service /etc/systemd/system
	install -d -m 750 --owner=www-data --group=www-data /var/www/.dave
	install -D -m 644 -v $(ROOT_DIR)/files/var/www/dave/config.yaml /var/www/.dave
	systemctl daemon-reload
...
radicale:
	...
	install -D -m 644 -v $(ROOT_DIR)/files/etc/radicale/config /etc/radicale
	install -D -m 644 -v $(ROOT_DIR)/files/etc/radicale/logging /etc/radicale
	install -D -m 644 -v $(ROOT_DIR)/files/etc/radicale/rights /etc/radicale
	install -D -m 644 -v $(ROOT_DIR)/files/etc/systemd/system/radicale.service /etc/systemd/system
	systemctl daemon-reload
...
phockup:
	...
	rsync -av $(ROOT_DIR)/files/opt/phockup/ /opt/phockup
	ln -sf /opt/phockup/phockup.py /usr/local/bin/phockup
	install -D -m 755 $(ROOT_DIR)/files/usr/local/bin/phockup-wrapper /usr/local/bin

diskplayer: mpd
	install -D -m 644 $(ROOT_DIR)/files/etc/udev/rules.d/100-floppy-change.rules /etc/udev/rules.d
	install -D -m 755 $(ROOT_DIR)/files/usr/local/bin/media_mount /usr/local/bin
	test -d /mnt/floppy || mkdir -p /mnt/floppy
	test -d /usr/local/share/diskplayer || mkdir -p /usr/local/share/diskplayer
	install -D -m 644 $(ROOT_DIR)/files/usr/local/share/diskplayer/bleep.mp3 /usr/local/share/diskplayer
...
motion:
	...
	install -D -m 644 $(ROOT_DIR)/files/etc/motion/motion.conf /etc/motion
	install -D -m 755 $(ROOT_DIR)/files/usr/local/bin/webcam.sh /usr/local/bin
...
cups: nginx_printer
	...
	install -D -m 644 $(ROOT_DIR)/files/etc/cups/cupsd.conf /etc/cups
