...
nginx_certificates:
	bash $(ROOT_DIR)/templates/cloudflare.sh
	...
	test -f /etc/nginx/pki/pki/ca.crt || install -D -m 644 -v $(ROOT_DIR)/files/etc/nginx/pki/pki/ca.crt /etc/nginx/pki/pki
	install -D -m 644 -v $(ROOT_DIR)/files/etc/nginx/pki/pki/crl.pem /etc/nginx/pki/pki

nginx_proxies:
	bash $(ROOT_DIR)/templates/nginx/sites/proxies.sh

nginx_sites:
	bash $(ROOT_DIR)/templates/nginx/sites/bank.sh
	bash $(ROOT_DIR)/templates/nginx/sites/ci.sh
	bash $(ROOT_DIR)/templates/nginx/sites/dav.sh
	bash $(ROOT_DIR)/templates/nginx/sites/default.sh
	bash $(ROOT_DIR)/templates/nginx/sites/git.sh
	bash $(ROOT_DIR)/templates/nginx/sites/netdata.sh
	bash $(ROOT_DIR)/templates/nginx/sites/podcasts.sh
	bash $(ROOT_DIR)/templates/nginx/sites/repo.sh
	bash $(ROOT_DIR)/templates/nginx/sites/sync.sh
	bash $(ROOT_DIR)/templates/nginx/sites/mood.sh

nginx_printer: nginx
	bash $(ROOT_DIR)/templates/nginx/sites/printer.sh

nginx_configs:
	install -D -m 644 -v $(ROOT_DIR)/files/etc/logrotate.d/nginx /etc/logrotate.d
	install -D -m 755 -v $(ROOT_DIR)/files/usr/local/bin/clicks_count /usr/local/bin
	install -D -m 644 -v $(ROOT_DIR)/files/etc/nginx/conf.d/log_format_json.conf /etc/nginx/conf.d
	install -D -m 644 -v $(ROOT_DIR)/files/etc/nginx/conf.d/traccar.conf /etc/nginx/conf.d
	install -D -m 644 -v $(ROOT_DIR)/files/etc/nginx/conf.d/limits.conf /etc/nginx/conf.d
