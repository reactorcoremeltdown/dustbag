...
deviceping:
	...
	install -D -m 755 $(ROOT_DIR)/files/usr/local/bin/deviceping /usr/local/bin
	@echo "$(ccgreen)Setting up deviceping completed$(ccend)"

deviceping_receiver:
	install -D -m 755 $(ROOT_DIR)/files/usr/local/bin/deviceping-receiver /usr/local/bin
	test -d /var/spool/api/deviceping || mkdir -p /var/spool/api/deviceping
	@echo "$(ccgreen)Setting up deviceping completed$(ccend)"

exported_graphs:
	test -d /opt/apps/graphs || mkdir -p /opt/apps/graphs
	install -D -m 755 $(ROOT_DIR)/files/usr/local/bin/grafana_pictures.sh /usr/local/bin
	@echo "$(ccgreen)Setting up graphs completed$(ccend)"

gotify:
	test -d /var/lib/gotify || mkdir -p /var/lib/gotify
	install -D -m 644 $(ROOT_DIR)/files/etc/systemd/system/gotify.service /etc/systemd/system
	systemctl enable gotify.service
	systemctl restart gotify.service
	bash $(ROOT_DIR)/templates/nginx/sites/gotify.sh
